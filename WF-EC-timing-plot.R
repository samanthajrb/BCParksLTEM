...
#     - Waterfowl survey timing each year at all sites
# ===========================
# ===========================
...
WF.months.expand = expand.grid(
  Study.Area = unique(WF.timing.months$Study.Area), 
  Year = WF.all.years)

# Identify years with no survey data per location                               
WF.months = WF.months.expand %>%
  left_join(WF.timing.months %>% select(Study.Area, Year, everything()), 
...
WF.timing.shapes = c("Circle" = 1, "Square" = 15)
WF.shp.labs = c("Circle" = str_wrap("Survey Completed", width = 12), "Square" = str_wrap("Representative Survey **", width = 12))
...
plot.filename = paste0("WF-EC-timing-plot-",
                       thedate,
                       ".png")
...
plot.v1 = ggplot(WF.data.timing,
  ...
  geom_tile(
    data = WF.months %>% filter(is.surveyed == 0),
    aes(x = Year, 
        y = 0, 
  ...
  theme_minimal() +
  labs(
    title = "Waterfowl Survey Timing (2013-2024)",
    x = "Year",
  ) +
  scale_x_continuous(
    breaks = WF.all.years, 
